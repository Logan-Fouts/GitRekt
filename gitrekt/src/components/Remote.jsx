      ...
      {remoteInfo && remoteInfo.length > 0 ? (
        <ul>
          {remoteInfo.map((branch, index) => (
            <li key={index} className="flex items-center space-x-2 py-2 border-gray-200">
              <svg className="w-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
const RemoteContent = ({ reponame, owner }) => {
  ...
  if (!remoteInfo) return <div>Loading...</div>;
